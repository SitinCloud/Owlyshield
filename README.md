...
Owlyshield is an open-source AI-driven antivirus engine written in [Rust](https://rust-lang.org). Static analysis as performed by AV is only able to detect known threats, explaining why hackers are adapting so quickly and ransom attacks surging. We provide an embedded behavioural analysis AI that is able to detect and kill ransomwares in their very early execution.

## :vulcan_salute: Open-source philosophy

We at [SitinCloud 🇫🇷](https://www.sitincloud.com) strongly believe that cybersecurity products should always be open-source:
...
Although commercial products or services can be directly purchased from us (feel free to [contact us](mailto:opensource@sitincloud.com) directly for any quotation that could suit your need), we think that our products should be distributed to end customer in an indirect way.
